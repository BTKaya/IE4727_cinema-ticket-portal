...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const seats = document.querySelectorAll(".seat.available, .seat.selected, .seat.booked");
    const confirmBtn = document.getElementById("confirmBooking");
    const selectedSeatsText = document.getElementById("selectedSeatsText");
    const totalPriceText = document.getElementById("totalPrice");

    function updateSelectionSummary() {
        const selectedSeats = [...document.querySelectorAll(".seat.selected:not(.static)")]
            .map(s => s.dataset.seat);
        const count = selectedSeats.length;
        const total = count > 0 ? count * SEAT_PRICE : 0;

        if (selectedSeatsText) {
            selectedSeatsText.textContent = count ? selectedSeats.join(", ") : "None";
        }
        if (totalPriceText) {
    ...
    function clearSeatErrors() {
        document.querySelectorAll(".seat.error").forEach(s => s.classList.remove("error"));
    }
    ...
    const blocks = layoutType === 1
        ? [[1,2],[3,4],[5,6],[7,8]]     // 8x8 → 2|2|2|2
        : [[1,2],[3,8],[9,10]];        // 8x10 → 2|6|2

    for (const row in rows) {
        const seats = rows[row];

        for (const [start, end] of blocks) {
        const blockSeats = seats.filter(s => s.num >= start && s.num <= end);

        const occ = blockSeats.map(s =>
            s.el.classList.contains("booked") ? 2 :
            s.el.classList.contains("selected") ? 1 : 0
        );

        // If block is exactly 2 seats → prevent 1 isolated seat
        if (occ.length === 2) {
            if ((occ[0] === 1 && occ[1] === 0) || (occ[0] === 0 && occ[1] === 1)) {
            blockSeats.forEach((s, i) => { if (occ[i] === 1) offenders.add(s.el); });
            }
            continue;
        }
        ...
        for (let i = 0; i < occ.length; i++) {
            ...
            const isolated = (i > 0 && i < occ.length - 1 && left && right)
                        || (i === 0 && right)
                        || (i === occ.length - 1 && left);

            if (isolated) {
            if (i > 0 && occ[i - 1] === 1) offenders.add(blockSeats[i - 1].el);
            if (i < occ.length - 1 && occ[i + 1] === 1) offenders.add(blockSeats[i + 1].el);
            }
        }
        }
    }
    ...
    if (confirmBtn) {
        ...
        if (offenders.length > 0) {
            ...
            alert("⚠ You cannot leave a single-seat gap. Please adjust highlighted seats.");
            return;
        }
        ...
        console.log("📤 Sending booking request:", { movieId, selectedSeats, date, time });

        const res = await fetch("save_booking.php", {
            method: "POST",
            headers: {"Content-Type": "application/json"},
            body: JSON.stringify({ movie_id: movieId, seats: selectedSeats, date, time })
        });
